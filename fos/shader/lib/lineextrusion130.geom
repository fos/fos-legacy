...
// One color for each vertex in the line
in vec4 vColor0[2];
// one width for each vertex in the line
in float vWidth[2];
...
void main()
{
    ...
    vec3 winCoord0, winCoord1;
    vec3 computedWinCoord0,computedWinCoord1, computedWinCoord2, computedWinCoord3;
    vec4 clipCoord0, clipCoord1, clipCoord2, clipCoord3;
    vec3 dir;
    vec2 perp;
    ...
    // the quad extrusion in screen-coordinates
    ...
    // Pass those along as clip-space coordinates for the fragment shader

    // first triangle
    gl_Position = clipCoord0; vColor1 = vColor0[0]; EmitVertex();
    gl_Position = clipCoord1; vColor1 = vColor0[0]; EmitVertex();
    gl_Position = clipCoord2; vColor1 = vColor0[1]; EmitVertex();
    EndPrimitive();
    
    // second triangle
    gl_Position = clipCoord0; vColor1 = vColor0[0]; EmitVertex();
    gl_Position = clipCoord2; vColor1 = vColor0[1]; EmitVertex();
    gl_Position = clipCoord3; vColor1 = vColor0[1]; EmitVertex();
    EndPrimitive();
