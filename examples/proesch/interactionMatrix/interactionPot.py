...
def init( ):
	...
	glEnable( GL_DEPTH_TEST )
        print 'GL_DEPTH_TEST', GL_DEPTH_TEST
	glShadeModel( GL_SMOOTH )
        print 'GL_SMOOTH', GL_SMOOTH
	glEnable( GL_LIGHTING )
        print 'GL_LIGHTING', GL_LIGHTING
        
	glEnable( GL_LIGHT0 )

        print 'First light', GL_LIGHT0
        
	glLightModeli( GL_LIGHT_MODEL_TWO_SIDE, 0 )

        print GL_LIGHT_MODEL_TWO_SIDE, 0
	glLightfv( GL_LIGHT0, GL_POSITION, [4, 4, 4, 1] )

        print GL_LIGHT0,  [4, 4, 4, 1]

        
	lA = 0.8
	glLightfv( GL_LIGHT0, GL_AMBIENT, [lA, lA, lA, 1] )

        print GL_LIGHT0,  [lA, lA, lA, 1]
        
	lD = 1
	glLightfv( GL_LIGHT0, GL_DIFFUSE, [lD, lD, lD, 1] )

        print GL_LIGHT0,  [lD, lD, lD, 1]
        
	lS = 1
	glLightfv( GL_LIGHT0, GL_SPECULAR, [lS, lS, lS, 1] )

        print GL_LIGHT0,  [lS, lS, lS, 1]
	...
	global mouseInteractor
	mouseInteractor = MouseInteractor( .1, 1 )
	global tkList
	tkList = glGenLists( 1 )
